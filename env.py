...
class SmartCustomerSupportEnv:
    ...
    def step(self, action: str) -> Dict[str, Any]:
        ...
        reward = float(evaluation.get("reward", 0.5))
    ...
    def _score_groups(
        ...
        coverage = len(matched) / total_groups if total_groups > 0 else 0.5
        coverage = float(f"{coverage:.6f}")
        coverage = max(0.05, min(coverage, 0.95))
