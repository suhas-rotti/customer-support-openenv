...
class SmartCustomerSupportEnv:
    ...
    def _evaluate_response(self, action: str) -> Dict[str, Any]:
        ...
        reward = (
            min(required_result["coverage"], 0.94) * 0.5
            + min(optional_result["coverage"], 0.94) * 0.1
            + min(empathy_score, 0.19)
            + min(length_score, 0.19)
        )
        ...
        if reward <= 0.0:
            reward = 0.02
        reward = max(0.01, min(reward, 0.94))
    ...
    def _score_groups(
        ...
        if coverage <= 0.0:
            coverage = 0.01
        coverage = max(0.01, min(coverage, 0.95))
