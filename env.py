...
class SmartCustomerSupportEnv:
    ...
    def step(self, action: str) -> Dict[str, Any]:
        ...
        assert 0.0 < reward < 1.0, f"INVALID REWARD DETECTED: {reward}"
    ...
    def _evaluate_response(self, action: str) -> Dict[str, Any]:
        ...
        length_score = 0.2 if 15 <= word_count <= 80 else 0.01
        empathy_score = 0.2 if empathy_result["matched"] else 0.01

        reward = (
            min(required_result["coverage"], 0.95) * 0.5
            + min(optional_result["coverage"], 0.95) * 0.1
            + min(empathy_score, 0.2)
            + min(length_score, 0.2)
        )

        if word_count < 5:
            reward = max(reward - 0.2, 0.01)

        if re.search(r"\bidk\b", normalized_action):
            reward = max(reward - 0.3, 0.01)
    ...
    def _score_groups(
        ...
        if total_groups == 0:
            ...
            coverage = len(matched) / total_groups

        coverage = max(0.01, min(coverage, 0.98))
