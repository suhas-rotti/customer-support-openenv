...
class SmartCustomerSupportEnv:
    ...
    def step(self, action: str) -> Dict[str, Any]:
        ...
        raw = float(evaluation["reward"])

        reward = max(0.01, min(raw, 0.99))
    ...
    def _evaluate_response(self, action: str) -> Dict[str, Any]:
        ...
        length_score = 0.2 if 15 <= word_count <= 80 else 0.0
        empathy_score = 0.2 if empathy_result["matched"] else 0.0

        reward = (
            min(required_result["coverage"], 0.99) * 0.5
            + min(optional_result["coverage"], 0.99) * 0.1
            + min(empathy_score, 0.99)
            + min(length_score, 0.99)
        )
